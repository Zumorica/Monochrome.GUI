            ...
            switch (Stretch)
            {
                case StretchMode.Scale:
                    handle.DrawTextureRect(texture,
                        UIBox2.FromDimensions(Vector2.Zero, PixelSize.ToVector2()));
                    break;
                ...
                {
                    var position = (PixelSize.ToVector2() - texture.Size() * _textureScale * UIScale) / 2;
                ...
                case StretchMode.KeepAspectCentered:
                {
                    ...
                    {
                        position = (PixelSize.ToVector2() - size) / 2;
                    }
                ...
                case StretchMode.KeepAspectCovered:
                    ...
                    var (scaleX, scaleY) = PixelSize.ToVector2() / texSize;
                    // Use whichever scale is greater.
                    var scale = Math.Max(scaleX, scaleY);
                    // Offset inside the actual texture.
                    var offset = (texSize - PixelSize.ToVector2()) / scale / 2f;
                    handle.DrawTextureRectRegion(texture, PixelSizeBox, UIBox2.FromDimensions(offset, PixelSize.ToVector2() / scale));
                    break;
                default:

...
namespace Monochrome.GUI.Controls
{
    public class TabContainer : Container
    {
        ...
        {
            return _tabData[tab].Name ?? GetChild(tab).Name ?? "No title";
        }
        ...
        protected override void ChildMoved(Control child, int oldIndex, int newIndex)
        {
            ...
            Debug.Assert(data.Control == child);
        ...
        {
            var total = Point.Zero;

            foreach (var child in Children)
            {
                if (child.Visible)
                {
                    total = PointHelper.ComponentMax(child.CombinedPixelMinimumSize, total);
                }
            }

            if (TabsVisible)
            {
                total += new Point(0, _getHeaderSize());
            }

            var panel = _getPanel();
            total += (panel?.MinimumSize ?? Vector2.Zero).ToPoint();

            return total.ToVector2() / UIScale;
        }

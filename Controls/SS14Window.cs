...
namespace Monochrome.GUI.Controls
{
    ...
    public class SS14Window : Control
    {
            ...
            AddChild(new VBoxContainer
            {
                ...
                Children =
                {
                    new PanelContainer
                    {
                        ...
                        CustomMinimumSize = new Vector2(0, HEADER_SIZE_Y),
                        Children =
                        {
                            new HBoxContainer
                            {
        ...
        protected internal override void KeyBindDown(GUIBoundKeyEventArgs args)
        {
            ...
            {
                DragOffsetTopLeft = args.PointerLocation / UIScale - Position;
                DragOffsetBottomRight = Position + Size - args.PointerLocation / UIScale;
            }
        ...
        {
            Open();
            LayoutContainer.SetPosition(this, new Vector2(0, (Parent.Size.Y - Size.Y) / 2));
        }

        // Prevent window headers from getting off screen due to game window resizes.

        protected override void Update(GameTime args)
        {
            var (spaceX, spaceY) = Parent.Size;
            if (Position.Y > spaceY)
            {
                LayoutContainer.SetPosition(this, new Vector2(Position.X, spaceY - HEADER_SIZE_Y));
            }

            if (Position.X > spaceX)
            {
                // 50 is arbitrary here. As long as it's bumped back into view.
                LayoutContainer.SetPosition(this, new Vector2(spaceX - 50, Position.Y));
            }
        }
